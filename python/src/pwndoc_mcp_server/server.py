...
class PwnDocMCPServer:
    ...
    def _handle_message(self, message: Dict) -> Optional[Dict]:
        ...
        # Notifications should NEVER receive a response per JSON-RPC 2.0 spec
        is_notification = msg_id is None

        handlers = {
            "initialize": self._handle_initialize,
            "initialized": lambda p: None,  # Notification, no response
            "notifications/initialized": lambda p: None,  # Alternative notification path
            "notifications/cancelled": lambda p: None,  # Cancel notification
            "notifications/progress": lambda p: None,  # Progress notification
            "cancelled": lambda p: None,  # Cancel notification (short form)
        ...
        if method in handlers:
            try:
                ...
                if result is None:  # Notification handler returned None
                    return None
            ...
            # Unknown method - only return error for requests, silently ignore notifications
            if is_notification:
